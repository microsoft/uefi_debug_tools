...
static bool
VerifyBuildId (
  _In_ PCSTR   elfFilePath,
  _In_ PVOID   pBldIdData,
  ...
  )
{
  ...
  if (  ReadFile (hElfFile, elfHeader, sizeof (elfHeader), &dwRead, nullptr)
     && (dwRead >= 64)
     && (elfHeader[4] == 2))                      // EI_CLASS: 2 = 64-bit (32-bit not supported)
  {
    // Parse 64-bit ELF header
    ULONG64  shoff     = *(ULONG64 *)&elfHeader[40];  // Section header offset
    WORD     shentsize = *(WORD *)&elfHeader[58];     // Section header entry size
    WORD     shnum     = *(WORD *)&elfHeader[60];     // Number of section headers
    WORD     shstrndx  = *(WORD *)&elfHeader[62];     // Section name string table index
    ...
    if (shstrndx < shnum) {
      ...
      if (ReadFile (hElfFile, shstrtabHeader, shentsize, &dwRead, nullptr) && (dwRead == shentsize)) {
        ULONG64  shstrtabOffset = *(ULONG64 *)&shstrtabHeader[24];
        shstrtabSize = *(ULONG64 *)&shstrtabHeader[32];

        if ((shstrtabSize > 0) && (shstrtabSize < 1024 * 1024)) {
          // Sanity check
    ...
    for (WORD i = 0; i < shnum && !buildIdMatches; i++) {
      ...
      if (ReadFile (hElfFile, sectionHeader, shentsize, &dwRead, nullptr) && (dwRead == shentsize)) {
        ...
        ULONG64  sh_offset = *(ULONG64 *)&sectionHeader[24];
        ULONG64  sh_size   = *(ULONG64 *)&sectionHeader[32];
...

{
  if (  (FilePath == nullptr)
     || (FilePath[0] == '\0')
     || (BaseAddress == 0)
     || (spSymbolProvider2 == nullptr)
     || ((pBldIdData == nullptr) && (bldIdSize != 0)))
  {
    return FALSE;
  }
